...
**不是。** Blackhole Diffusion 追求满秩正确性，不保证全局最短总环长。偏离是系统性的：从不比精确 MCB 更短，只会更长。1000 边级别 70% 的图基边膨胀 <1%，规模越大、环长跨度越大，偏离越明显。如果你需要严格 MCB 且 igraph 可用，请直接用 igraph。如果你在无法安装 igraph 的环境中需要一份正确可用的环基，Blackhole Diffusion 就是为你准备的。详见**基准报告**和 `CAUTION.md`。
